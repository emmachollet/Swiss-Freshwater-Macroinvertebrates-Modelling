...
# Make tables
# ECR: Some modifications for presentation
# nice.taxa <- c("Occurrence.Athericidae", "Occurrence.Psychomyiidae")
# tmp.table <- df.pred.perf[which(df.pred.perf$Taxa %in% nice.taxa), c("Taxa", 
#                             # "Prevalence", 
#                             "glm", "UF0","CF0","gamLoess", "svmRadial", "rf", "ANN")]
#nice.taxa <- c("Occurrence.Athericidae", "Occurrence.Psychomyiidae")
tmp.table <- df.pred.perf[ c("Taxa", 
                                                                     # "Prevalence", 
                                                                     "glm", "UF0","CF0","gamLoess", "svmRadial", "rf", "ANN")]
# colnames(tmp.table) <- c("Taxa", "glm", "UF0","CF0","gamLoess", "svmRadial", "rf", "ANN")
# tmp.table$Taxa <- c("Athericidae", "Psychomyiidae")
