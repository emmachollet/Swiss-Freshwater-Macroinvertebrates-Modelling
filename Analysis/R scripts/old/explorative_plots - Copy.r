...
library(sf) # to read GIS data (shape files)
library(caret) # comprehensive framework to build machine learning models
library(ggpubr) # to make nice arrangement of nice plots
...
if(BDM == TRUE) {
    # don't change anything, take all env. fact
} else {
    # remove InS env. fact.
...
# env.fact <- c("GEO", "Canton", "IAR", "temperature")
...
print(proc.time()-ptm)
# Producing All_pdf -> 846 sec = 14 min
# Producing BDM_pdf -> 132 sec = 2 min
