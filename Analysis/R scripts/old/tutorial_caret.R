## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##
## ---- Training of Machine Learning models on processed data ----
## 
...
# library(randomForest) # ML model (regression and classification)
# library(caTools) # to split data sets
# library(pdp) # for partial dependence plots
# library(vip) # for variable importance plots
# library(ROCR) # for ROC curves plots
...
dir.data          <- "../../Data/Processed data/" 

file.inv.data     <- "Invertebrate data/All_occ_data_2020-06-25.dat"
file.env.data     <- "Environmental data/environmental_data_2021-05-26.dat"

# Read data
data.inv          <- read.delim(paste(dir.data,file.inv.data,sep=""),header=T,sep="\t", stringsAsFactors=FALSE) 
data.env          <- read.delim(paste(dir.data,file.env.data,sep=""),header=T,sep="\t", stringsAsFactors=FALSE)
...
env.fact <- c("IAR", "saprobic_cond", "urban_area", "velocity", "temperature") # inspired from Bogdan's selection in 2020
...
lmProfile <- rfe(x=trainData[, env.fact], y=trainData[, occ.taxa], # receives the output of the rfeControl() as values
                 sizes = c(1:3, 5), # what all model sizes (the number of most important features) the rfe should consider.
                 rfeControl = ctrl)
...
# HINT to modify col names

ttolower() # replace strings in lower letters
gsub() # replace some char by other
