...
data[BDMind[which(is.na(data[BDMind,"InS_LBUKMAT"])&!is.na(data[BDMind,"BREITENVAR"]))],"InS_LBUKMAT"] <- 0  # 0 means no LBUKVER because no LBUKVER
...
data[which(is.na(data$LBUKVER)),"LBUKVER"]  <- 1 
...
data[BDMind[which(is.na(data[BDMind,"InS_RBUKMAT"]))],"InS_RBUKMAT"] <- 0  # 0 means no LBUKVER because no LBUKVER
...
data[which(is.na(data$RBUKVER)),"RBUKVER"]  <- 1 
