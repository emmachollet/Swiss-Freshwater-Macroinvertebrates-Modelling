...
data[which(is.na(data$LBUKVER)&!is.na(data$EINDOL)),"LBUKVER"]  <- 1  #xxx
...
data[BDMind[which(is.na(data[BDMind,"InS_RBUKMAT"]))],"InS_RBUKMAT"] <- 0  # 0 means no LBUKVER because no LBUKVER xxx
...
data[which(is.na(data$RBUKVER)&!is.na(data$EINDOL)),"RBUKVER"]  <- 1  #xxx
